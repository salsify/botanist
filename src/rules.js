 ...
 */
export function applyRules(ast, rules) {
  if (isObject(ast)) {
    let result = Object.create(null);
    for (let key of Object.keys(ast)) {
      result[key] = applyRules(ast[key], rules);
    }
    return applyMatchingRule(result, rules);
  } else if (isArray(ast)) {
    let result = ast.map(node => applyRules(node, rules));
    return applyMatchingRule(result, rules);
  } else {
    return ast;
  }
...
function applyMatchingRule(node, rules) {
  let context = new Context();
  for (let rule of rules) {
    if (rule.match(node, context)) {
      return rule.transform.call(null, context.expose(), node);
    }
  }
  return node;
}
